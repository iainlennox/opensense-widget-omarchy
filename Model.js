...
//
// No QML or shell dependencies: every function here can be exercised with
// plain Node (`node -e "..."`) or the bundled test runner. Keep it that way.
...
var Model = (function () {
  ...
  function displaySpeed(media) {
    ...
    if (!bps) return media || "";
    return formatBandwidth(bps).replace("bps", "bps");
  }
  ...
  // Single-colour helpers. The original picks a per-metric colour, and the
  // overall health status is computed from a score.
  ...
  // The bar size hint: to avoid drawing a scale that jumps on every sample,
  // the panel holds a stable `scale` per interface; this helper seeds the
  // first sample.
